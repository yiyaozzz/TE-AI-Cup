...
def find_logo(image, logo_template_path, threshold=0.8):
    ...
    for scale in np.linspace(0.2, 1.0, 20)[::-1]:
        ...
        resized = cv2.resize(logo_template, None, fx=scale,
...
def rotate_image_to_landscape(image, logo_template_path):
    ...
    correctly_oriented_image = rotate_image_until_logo_matches(
        image_np, logo_template_path)
    return correctly_oriented_image


def convert_pdf_to_images(pdf_path):
    # Added dpi for better image quality
...
def process_pdf_to_landscape(pdf_path, output_dir, logo_template_path):
    ...
    landscape_images = [rotate_image_to_landscape(
        image, logo_template_path) for image in images]
