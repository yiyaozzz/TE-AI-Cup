...
def write_to_excel(data, excel_path, row_count):
    ...
    if os.path.exists(excel_path):
        ...
        df = df.astype('object')  # Convert all columns to object type to avoid dtype issues
    else:
    ...
    if len(df) < row_count:
        ...
        additional_rows = pd.DataFrame([pd.Series([None]*len(COLUMNHEADING), index=COLUMNHEADING)] * (row_count - len(df)))
    ...
    for key in ['Prod Order', 'Material Number', 'Material Description', 'Order Type', 'Plant']:
        df[key] = [data.get(key, "")] * row_count  # Overwrite and fill down the column
    
    # Populate 'Posting date' and 'Entry Date' with today's date
    today_date = datetime.datetime.now().strftime("%m/%d/%y")  # Get today's date in the specified format
