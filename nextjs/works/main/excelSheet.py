import pandas as pd
import json
from main.variables import OPRID, COLUMNHEADING, SCRAP_CODE_MAPPING
import main.excel_ocr as excel_ocr
import os
        ...
        for row_id, columns in contents.items():
            ...
            if scrap_details and isinstance(scrap_details, list) and scrap_details != ['N/A', '0']:
                for item in scrap_details:
                    if isinstance(item, dict) and 'name' in item and 'value' in item:
                        ...
                        if name not in ["ว", "Samples", "Tensile Test"]:
            ...
            if scrap_quantities:
                for description, quantity in scrap_quantities.items():
                    ...
                    all_rows.append([work_center, operation, scrap_code, description,
                                    op_good_qty if op_good_qty is not None else 0, quantity] + [None] * (len(COLUMNHEADING) - 6))
            else:
                all_rows.append([work_center, operation, None, '',
                                op_good_qty if op_good_qty is not None else 0, 0] + [None] * (len(COLUMNHEADING) - 6))
...
def sheetmain(firstPage_imagePath, restPages_jsonPath, uidVal):
    ...
    excel_ocr.process_image_for_ocr(
        firstPage_imagePath, excel_output_file, json_row_count)
